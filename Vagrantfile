...
KEYCLOAK_CLUSTERED_CONTAINER_ARGS = "-p 8080:8080 -p 7800:7800 -p 8443:8443 -e KEYCLOAK_ADMIN=#{KEYCLOAK_ADMIN} -e KEYCLOAK_ADMIN_PASSWORD=#{KEYCLOAK_ADMIN_PASSWORD} -e KC_DB=#{KC_DB} -e KC_DB_URL_HOST=#{DATABASE_IP} -e KC_DB_URL_DATABASE=#{KC_DB_URL_DATABASE} -e KC_DB_USERNAME=#{KC_DB_USERNAME} -e KC_DB_PASSWORD=#{KC_DB_PASSWORD} -e KC_LOG_LEVEL=#{KC_LOG_LEVEL}"
